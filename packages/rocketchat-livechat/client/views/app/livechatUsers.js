...
Template.livechatUsers.helpers({
	managers() {
		var agents = _.pluck(LivechatDepartmentAgents.find({}).fetch(), 'agentId');
		return ManagerUsers.find({_id:{$in:agents}}, { sort: { name: 1 } });
	},
	agents() {
		var agents = _.pluck(LivechatDepartmentAgents.find({}).fetch(), 'agentId');							
		return AgentUsers.find({_id:{$in:agents}}, { sort: { name: 1 } });
	},
	emailAddress() {
...
Template.livechatUsers.onCreated(function() {
	...
	this.subscribe('livechat:departmentAgentsbyAgentId');
});
